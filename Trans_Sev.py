...
def nodo_o():
	...
	x = 1
#	bucle que me recorre bicis hasta llegar al punto del nodo seleccionado para tomar los datos del nodo		
...
# pendiente convaersion de coordenadas UTM a decimales
	...
	d = 2*r*asin(sqrt(sin(c*(lat2-lat1)/2)**2 + cos(c*lat1)*cos(c*lat2)*sin(c*(lng2-lng1)/2)**2))
	return d
...
#~ #consultamos la api de tussam y creamos una lista con las lineas que pasamos al template
#~ @route('/lista_linea_destino')
#~ def lista_linea_destino():
	#~ cliente = Client('http://www.infobustussam.com:9001/services/estructura.asmx?wsdl',retxml=True)
	#~ lineas = cliente.service.GetLineas()
	#~ lin = etree.fromstring(lineas)
	#~ ns ={"ns":"http://tempuri.org/","soap":"http://schemas.xmlsoap.org/soap/envelope/" } #definimos el namespace
	...
	#~ for i in xrange(lg):
		...
		#~ dicc["nombre"] = nombre[i].text
		#~ lbl.append(dicc)
	#~ return template ('lista_linea_destino',lbl=lbl,lg=lg)
	#~ 
...
def resumen_bus():
	...
	y_o = float(lng_o)
	y_d = float(lng_d)
	dist=distancia(x_o,y_o,x_d,y_d)
	return template ('resumen_bus',linea_o=linea_o, nm_o=nm_o, nm_d=nm_d ,nodo_o=nodo_o, nodo_d=nodo_d, lat_o=lat_o, lng_o=lng_o, lat_d=lat_d, lng_d=lng_d, dist=dist)
